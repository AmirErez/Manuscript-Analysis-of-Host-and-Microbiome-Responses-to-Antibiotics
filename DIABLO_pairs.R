#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#
# DIABLO Multi-Omics Integration Pipeline (Adapted for 5x1 Design)
#
...
# --- Load Metadata ---
metadata <- read_tsv(PATH_METADATA) %>%
  mutate(ID = as.character(ID))

# --- Load Transcriptome Data (Features as Rows) ---
transcriptome_raw <- read_tsv(PATH_TRANSCRIPTOME)
...
transcriptome_transposed <- transcriptome_raw %>%
  column_to_rownames("gene_id") %>% # gene_name column here is actually ENSMUSG IDs
  t() %>%
  as.data.frame()
...
run_diablo_analysis <- function(X_data, Y_variable, analysis_name, n_components = 2) {
  ...
  # This creates a copy for plotting so the original object remains untouched
  plot_model <- final.model
  
  # Function to extract the last part of the taxonomy string
  # Handles strings separated by ";", "|", or "."
  clean_taxa_names <- function(names_vec) {
    # Split by common delimiters and take the last element
    # sapply(strsplit(names_vec, "[;|.]"), function(x) tail(x, 1))
    sapply(strsplit(names_vec, "[;|.]"), function(x) paste(tail(x, 2), collapse = ";"))
  }
  
  # 1. Update the column names in the data matrix
  colnames(plot_model$X$microbiome) <- clean_taxa_names(colnames(plot_model$X$microbiome))
  
  # 2. Update the names in the loadings (for all components)
  # This is critical for circosPlot to find the features
  for(i in 1:ncol(plot_model$loadings$microbiome)) {
    rownames(plot_model$loadings$microbiome) <- clean_taxa_names(rownames(plot_model$loadings$microbiome))
  }
  
  # 3. Update the names in the list.keepX attribute
  # mixOmics uses this to know which variables were selected
  if(!is.null(plot_model$names$colnames$microbiome)) {
    plot_model$names$colnames$microbiome <- clean_taxa_names(plot_model$names$colnames$microbiome)
  }
  ...
  print(plotDiablo(plot_model, ncomp = 1, legend = TRUE, title = analysis_name))
  dev.off()
  
  cat("Saving circos plot as SVG...\n")
  svglite(paste0(output_prefix, "_circos_plot.svg"), width = 10, height = 10)
  circosPlot(plot_model, cutoff = 0.7, line = TRUE,
  ...
  cimDiablo(plot_model, margin = c(8, 12))
  dev.off()
  
  svglite(paste0(output_prefix, "_heatmap_grouped.svg"), width = 20, height = 25)
  cimDiablo(plot_model, margin = c(8, 12), cluster = "col")
  dev.off()
...
# #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# # ANALYSIS 1: Compare 5 Antibiotics *within* PO
# #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# ANALYSIS 2: Compare specific Drug-Treatment vs. matched PBS
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
cat("\n\n--- Starting Analysis 2: Specific Drug vs Matched PBS ---\n")
...
comparisons <- list(
  ...
  # list(drug1 = "Met", drug2 = "PBS", trt = "PO", name = "Compare_Met-PO_vs_PBS-PO"),
  # list(drug1 = "Met+Van", drug2 = "PBS", trt = "PO", name = "Compare_Met+Van-PO_vs_PBS-PO"),
  # list(drug1 = "Met+Neo", drug2 = "PBS", trt = "PO", name = "Compare_Met+Neo-PO_vs_PBS-PO")
)
